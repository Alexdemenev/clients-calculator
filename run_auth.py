"""
Запуск приложений с авторизацией
Использует multiprocessing вместо threading для избежания проблем с сигналами
"""

import subprocess
import sys
import time
import multiprocessing
import os


def run_streamlit():
    """Запускает Streamlit приложение для авторизации"""
    subprocess.run(
        [
            sys.executable,
            "-m",
            "streamlit",
            "run",
            "app/streamlit_auth.py",
            "--server.port=8501",
            "--server.headless=true",
        ]
    )


def run_dash():
    """Запускает Dash приложение"""
    time.sleep(2)  # Небольшая задержка для запуска Streamlit
    from app.dash_customer import app
    import os

    # Отключаем debug и reloader при запуске в отдельном процессе
    # Это необходимо, так как сигналы работают только в главном потоке
    debug_mode = os.getenv("DASH_DEBUG", "False").lower() == "true"
    app.run(
        debug=debug_mode,
        port=8050,
        host="127.0.0.1",
        use_reloader=False,  # Отключаем reloader для работы в отдельном процессе
    )


if __name__ == "__main__":
    print("🚀 Запуск приложений...")
    print("📝 Streamlit (авторизация): http://localhost:8501")
    print("📊 Dash (приложение): http://localhost:8050")
    print("\n⚠️  Сначала войдите через Streamlit, затем перейдите к Dash приложению\n")

    # Запускаем Streamlit в отдельном процессе
    streamlit_process = multiprocessing.Process(target=run_streamlit, daemon=True)
    streamlit_process.start()

    # Запускаем Dash в основном процессе
    try:
        run_dash()
    except KeyboardInterrupt:
        print("\n👋 Остановка приложений...")
        streamlit_process.terminate()
        streamlit_process.join()
